...
<body>
  ...
  <main id="main" class="main">
	...
	$TechnicianName='';
	$resultTN = $db_handle->runQuery("SELECT Name FROM `technicians` WHERE `Email` ='$email'");
	if(!empty($resultTN)){
		foreach ($resultTN as $r) {
	...
	?>
    <div class="pagetitle">
      <h1 style="font-weight: 900;">View Orders</h1><br>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><p  style="color:red; font-weight:bold; font-size:18px;">Technician Name :</p></li>
          <li ><p  style="color:red; font-weight:bold; font-size:18px;"><?php  echo ' '. $TechnicianName; ?></p></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
	<div id="response"></div>
    ...
    <section class="section">
	 ...
	 <div class="card" >
        ...
        <form class="form-inline" method="post" action="" style="margin-left:150px;">
          <label for="fromDate" class="col-form-label col-md-1"></label>
		  <br><br><br>
        ...
        <table>
			...
			<tbody>
				<?php
				$ID_Tech='';
				$resultT = $db_handle->runQuery("SELECT ID FROM `technicians` WHERE `Email` ='$email'");
				if(!empty($resultT)){
				...
				$queryA="SELECT customerappoinments.*,techniciansassigned.Status AS TechStatus FROM `customerappoinments` 
				INNER JOIN techniciansassigned ON techniciansassigned.JobNumber = customerappoinments.AppoinmentID 
				WHERE techniciansassigned.`TechnicianID`='$ID_Tech'";
				$resultA = $db_handle->runQuery($queryA);   //GROUP BY customerappoinments.`AppoinmentID`   ORDER BY customerappoinments.`AppoinmentID` DESC
				$i=0;
				$Status=0;
				$VehicleType='';
				if(!empty($resultA)){
					foreach ($resultA as $r) {
						...
						if($Status==0){
							...
							<td><a href="#" class="confirm-link" data-id="<?php echo $r['AppoinmentID']; ?>" style="color:red;">Finish</a></td>
							<?php 
						}else{
							?>
							<td ><a href="#" id="" class="" style="color:#00ff00;">Finished</a></td>
							<?php
						}
