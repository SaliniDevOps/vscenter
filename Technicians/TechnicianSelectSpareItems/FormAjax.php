...
<script type="text/javascript">
	...
	function selectname(SpareCode,SpareName,ID,AvalQTY){
		$("#ItemCode").val(SpareCode);
		$("#ItemName").val(SpareName);
		$("#HiddenID").val(ID);
		$("#availableQty").val(AvalQTY);
    ...
    $(function() {
		$("#Add_Button_SparePart").click(function() {
			...
			if(jobNumber==''){
				...
				$(".alert2").fadeIn(100).html(' <span class="closebtn" onclick="this.parentElement.style.display="none";"></span>Select Item');
				$(".alert2").fadeIn().delay(2000).fadeOut();
			}	else if(QTY==''){
				$(".alert3").fadeIn(100).html(' <span class="closebtn" onclick="this.parentElement.style.display="none";"></span>Enter QTY');
				$(".alert3").fadeIn().delay(2000).fadeOut();
			}	
