...
<?php   
	...
	if(!empty($_POST["Tradname_key"])) 
		...
		if(($_POST["Tradname_key"])!=" "){
				...
				<?php
					if(!empty($result1)){
						foreach ($result1 as $rowDep1) {
							...
							?>
							<a href="#" class="TradeName_link" id="<?php echo $ID;?>"><li onClick="selectname('<?php echo $SpareName;?>','<?php echo $SpareNameOri;?>','<?php echo $ID;?>','<?php echo $AvalQTY;?>');"><?php echo $SpareName;?></li></a>
							<?php
						}
					}
					else{
							?>
							<a href="#" class="TradeName_link" id="<?php echo $ID;?>"><li onClick="selectname('<?php echo $SpareName;?>','<?php echo $SpareNameOri;?>','<?php echo $ID;?>','<?php echo $AvalQTY;?>');">Not Match</li></a>
							<?php
						}
				?>
			</ul>
			...
			$Tradname_key=$_POST["Tradname_key"];
			$result1 = $db_handle->runQuery("SELECT spareparts.*,stock.`AvalQTY` FROM `spareparts` 
