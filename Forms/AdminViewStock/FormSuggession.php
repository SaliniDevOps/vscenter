...
<?php   
	...
	if(!empty($_POST["Tradname_key"])) 
		...
		if(($_POST["Tradname_key"])!=" "){
			...
			$result1 = $db_handle->runQuery("Select `spareparts`.`SpareName`,`spareparts`.`ItemCode`,`spareparts`.`Price`,`spareparts`.`ID`,`stock`.`AvalQTY`
			from `stock` 
			INNER JOIN  spareparts ON stock.SpareID = spareparts.ID
			Where `spareparts`.`SpareName` LIKE '$Tradname_key%' LIMIT 10");	
				...
				<?php
					if(!empty($result1)){
						foreach ($result1 as $rowDep1) {
							...
							$ID=$rowDep1["ID"];	
							?>
							<a href="#" class="TradeName_link" id="<?php echo $ID;?>"><li onClick="selectname('<?php echo $SpareName;?>');"><?php echo $SpareName;?></li></a>
							<?php
						}
					}
					else{
							?>
							<a href="#" class="TradeName_link" id="<?php echo $ID;?>"><li onClick="selectname('<?php echo $SpareName;?>');">Not Match</li></a>
							<?php
						}
				?>
			</ul>
