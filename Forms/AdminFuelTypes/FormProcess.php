...
<?php
	...
	if(isset($_POST["Insert_txtID"]))
	{
		...
		if (empty($ID)) {
			$resultA = $db_handle->runQuery("SELECT * FROM `fueltype` WHERE `FuelType`='$SpareName'"); 
			if($resultA instanceof mysqli_result && $resultA->num_rows > 0){
				...
				$result = $db_handle->insertQuery("INSERT INTO `fueltype` (`FuelType`) Values('$SpareName')");
			...
			$resultD = $db_handle->runQuery("SELECT * FROM `fueltype` WHERE `FuelType`='$SpareName' AND `ID`!='$ID'"); 
			if($resultD instanceof mysqli_result && $resultD->num_rows > 0){
				...
				$resultB = $db_handle->updateQuery("Update `fueltype` Set `FuelType`='$SpareName' WHERE `ID`='$ID'");
				...
				<table class="table table-bordered" style="max-width: 600px; margin: auto;">
					...
					<thead>
						<tr>
							...
							<th scope="col" class="center-align" style="background-color: #204060; color: white;">Fuel Type</th>
						</tr>
					</thead>
					<tbody>
					<?php
