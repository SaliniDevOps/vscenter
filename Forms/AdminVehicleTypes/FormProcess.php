...
<?php
	...
	if(isset($_POST["Insert_txtID"]))
	{
		...
		if (empty($ID)) {
			$resultA = $db_handle->runQuery("SELECT * FROM `vehicletype` WHERE `VehicleType`='$SpareName'"); 
			if($resultA instanceof mysqli_result && $resultA->num_rows > 0){
				...
				$result = $db_handle->insertQuery("INSERT INTO `vehicletype` (`VehicleType`) Values('$SpareName')");
			...
			$resultD = $db_handle->runQuery("SELECT * FROM `vehicletype` WHERE `VehicleType`='$SpareName' AND `ID`!='$ID'"); 
			if($resultD instanceof mysqli_result && $resultD->num_rows > 0){
				...
				$resultB = $db_handle->updateQuery("Update `vehicletype` Set `VehicleType`='$SpareName' WHERE `ID`='$ID'");
					...
					<thead>
						<tr>
						   <th scope="col" class="center-align">Delete</th>
							...
							<th scope="col" class="center-align">Vehicle Type</th>
						</tr>
					</thead>
					<tbody>
					<?php
					$queryA="SELECT * FROM `vehicletype` ORDER BY `ID`";
