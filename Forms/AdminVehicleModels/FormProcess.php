...
<?php
	...
	if(isset($_POST["Insert_txtID"]))
	{
		...
		if (empty($ID)) {
			$resultA = $db_handle->runQuery("SELECT * FROM `vehiclemodels` WHERE `VehicleModels`='$SpareName'"); 
			if($resultA instanceof mysqli_result && $resultA->num_rows > 0){
				...
				$result = $db_handle->insertQuery("INSERT INTO `vehiclemodels` (`VehicleModels`) Values('$SpareName')");
			...
			$resultD = $db_handle->runQuery("SELECT * FROM `vehiclemodels` WHERE `VehicleModels`='$SpareName' AND `ID`!='$ID'"); 
			if($resultD instanceof mysqli_result && $resultD->num_rows > 0){
				...
				$resultB = $db_handle->updateQuery("Update `vehiclemodels` Set `VehicleModels`='$SpareName' WHERE `ID`='$ID'");
				...
				<table class="table table-bordered" style="max-width: 600px; margin: auto;">
					...
					<thead>
						<tr>
							...
							<th scope="col" class="center-align"style="background-color: #204060; color: white; ">Vehicle Models</th>
						</tr>
					</thead>
					<tbody>
					<?php
					$queryA="SELECT * FROM `vehiclemodels` ORDER BY `ID`";
					...
					if(!empty($resultA)){
						foreach ($resultA as $r) {
							...
							<td><?php echo $r['VehicleModels'];?></td>
							</tr>
							<?php
						}
