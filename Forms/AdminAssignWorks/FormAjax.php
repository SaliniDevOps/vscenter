 <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
 <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
 
<script type="text/javascript">
    $(document).ready(function () {
  
		$(".delete").click(function () {
			var element = $(this);
			var del_id = element.attr("id");

			if (confirm("Are you sure you want to delete this?")) {
				$.ajax({
					type: "POST",
					url: "FormDelete.php",
					data: { id: del_id }, // Use object literal for data
					success: function (html) {
					$("#displaydelete").html($(html)); // Treat response as jQuery object
					
					document.getElementById('jobNumber').focus();
					document.getElementById('cmbTechnician').value='';
					document.getElementById('txtTechnicianID').value='';
					document.getElementById('jobNumber').value='';
				 
					},
				});
				$(this).parents("#show").animate({ backgroundColor:"#003"},"slow")
				.animate({ opacity:"hide"},"slow");
			}
		});
	});
</script>
<script type="text/javascript">
	
    //Update script
 	$(document).ready(function(){
		$('a[class="Update"]').click(function(){
			var Row_ID = $(this).val();
			var Row_ID = $(this).attr("id");
			
			if(Row_ID){
				$.ajax({
					type:'POST',
					url: "FormUpdate.php",
					data:'Row_ID_update='+Row_ID,
						success:function(html){
						$('#Row_ID_update').html(html);
						
						var type_ID = $("#type_ID_t").val();
							 
						document.getElementById('txtServiceType').value=type_ID;
						document.getElementById('txtServiceTypeID').value=Row_ID;
						
							
						//const input=document.getElementById('txtDays');
						input.focus();
						input.select();
						
					}
				}); 
			}else{
				$('#link_ShowData').html('<input type="text" value="g" >');
				$('#city').html('<input type="text" value="e" >'); 
			}
		});   
	});	
 
</script>
 
 <script type="text/javascript">
    $(function() {
		$("#Add_Button").click(function() {
			var jobNumber = $("#jobNumber").val();
			var cmbTechnician = $("#cmbTechnician").val();
			var txtTechnicianID = $("#txtTechnicianID").val();
			
		
			var dataString = 'Insert_cmbTechnician='+ cmbTechnician + '&jobNumber='+ jobNumber; 
							
			if(jobNumber==''){
				$(".textalert1").fadeIn(100).html(' <span class="closebtn" onclick="this.parentElement.style.display="none";"></span>Select the Job Number');
				$(".textalert1").fadeIn().delay(2000).fadeOut();
				document.getElementById('jobNumber').focus(); 
				
			}else if(cmbTechnician==''){
				$(".textalert2").fadeIn(100).html(' <span class="closebtn" onclick="this.parentElement.style.display="none";"></span>Select the Technician');
				$(".textalert2").fadeIn().delay(2000).fadeOut();
				document.getElementById('cmbTechnician').focus(); 
				
			}else{
			
				$.ajax({
					type: "POST",
					url: "FormProcess.php",
					data: dataString,
					cache: false,
					success: function(html){

						$("#Insert_Technicians").html(html);
						document.getElementById('jobNumber').focus();
						document.getElementById('cmbTechnician').value='';
						document.getElementById('txtTechnicianID').value='';
						document.getElementById('jobNumber').value='';
						
					}
				});
			}
			return false;
	    });
	});
	</script>
    
	

	
